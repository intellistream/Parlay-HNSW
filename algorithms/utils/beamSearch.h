...
const QueryParams &QP, bool use_filtering = false) {
    ...
    std::vector<id_dist> new_frontier(
        2 * std::max<size_t>(beamSize, starting_points.size()) + G.max_degree());
    ...
    while (remain > offset && num_visited < QP.limit) {
        ...
        if (use_filtering && frontier_full) {
            for (auto a : pruned) {
                if (frontier_full && Q_Points[a].distance(qp) >= filter_threshold)
                    continue;
        ...
        // sufficiently full. This iproves performance for higher accuracies (e.g.
        // beam sizes of 100+)
...
const QueryParams &QP, bool stats = true) {
    ...
    if (beamElts.size() < QP.k) {
        std::cout << "Error: for point id " << p.id() << " beam search returned "
                  << beamElts.size() << " elements, which is less than k = " << QP.k
                  << std::endl;
        abort();
    }
...
const QueryParams &QP) {
    ...
    if (use_rerank) {
        auto [pairElts, dist_cmps] = filtered_beam_search(
                                         G, qp, Q_Base_Points, qp, Q_Base_Points, starting_points, QP, false);
        auto [beamElts, visitedElts] = pairElts;
...
const indexType starting_point, const QueryParams &QP) {
    ...
    parlay::parallel_for(0, Query_Points.size(), [&](size_t i) {
        auto ngh_dist = beam_search_rerank(
                            Query_Points[i], Q_Query_Points[i], QQ_Query_Points[i], G, Base_Points,
                            Q_Base_Points, QQ_Base_Points, QueryStats, starting_points, QP);
        all_neighbors[i] = parlay::map(ngh_dist, [](auto &p) {
            return p.first;
        });
    });
